...
class BasketTrader:
    def run(self, state: TradingState, product: str, data: dict):
        ...
        gift_basket_position = state.position["GIFT_BASKET"] if product in state.position else 0
        gift_basket_limit_width = 60
        if sum_of_basket_products < gift_basket_mid_price:
            orders["GIFT_BASKET"].append(Order("GIFT_BASKET", gift_basket_mid_price, -gift_basket_limit_width - gift_basket_position))
        else:
            orders["GIFT_BASKET"].append(Order("GIFT_BASKET", gift_basket_mid_price, gift_basket_limit_width - gift_basket_position))

        chocolate_position = state.position["CHOCOLATE"] if product in state.position else 0
        chocolate_limit_width = 250
        chocolate_avg_weight = sum(data["CHOCOLATE"]["prev_weights"]) / len(data["CHOCOLATE"]["prev_weights"])
        if chocolate_weight > chocolate_avg_weight:
            orders["CHOCOLATE"].append(Order("CHOCOLATE", chocolate_mid_price, -chocolate_limit_width - chocolate_position))
        else:
            orders["CHOCOLATE"].append(Order("CHOCOLATE", chocolate_mid_price, chocolate_limit_width - chocolate_position))
        
        strawberries_position = state.position["STRAWBERRIES"] if product in state.position else 0
        ...
        if strawberries_weight > strawberries_avg_weight:
            orders["STRAWBERRIES"].append(Order("STRAWBERRIES", strawberries_mid_price, -strawberries_limit_width - strawberries_position))
        else:
            orders["STRAWBERRIES"].append(Order("STRAWBERRIES", strawberries_mid_price, strawberries_limit_width - strawberries_position))

        roses_position = state.position["ROSES"] if product in state.position else 0
        roses_limit_width = 60
        roses_avg_weight = sum(data["ROSES"]["prev_weights"]) / len(data["ROSES"]["prev_weights"])
        if roses_weight > roses_avg_weight:
            orders["ROSES"].append(Order("ROSES", roses_mid_price, -roses_limit_width - roses_position))
        else:
            orders["ROSES"].append(Order("ROSES", roses_mid_price, roses_limit_width - roses_position))
